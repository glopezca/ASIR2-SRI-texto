...
> 🧭 **ANTES DE TRABAJAR CON AUDIO Y VÍDEO DIGITAL**
>
> Un **códec** codifica y decodifica contenido; un **contenedor multimedia** agrupa pistas, metadatos y otros elementos. El **bitrate** expresa la cantidad de bits procesados o transmitidos por unidad de tiempo y los **FPS (Frames Per Second)** indican fotogramas por segundo. **Streaming** permite consumir contenido mientras continúa la recepción. **FFmpeg** proporciona herramientas de procesamiento; **RTMP** se utiliza en determinados flujos de ingestión y **HLS (HTTP Live Streaming)** distribuye contenido segmentado mediante HTTP.
...
```bash
curl -s http://radio.juandecolonia.jc:8000/status-json.xsl
```
...
ffmpeg \
  ...
  -b:a 128k \
  -f mp3 \
  icecast://source:CONTRASEÑA_SOURCE@radio.juandecolonia.jc:8000/radio.mp3
```
...
```text
http://radio.juandecolonia.jc:8000/radio.mp3
```
...
```bash
nc -vz radio.juandecolonia.jc 8000
```
...
```text
http://podcast.juandecolonia.jc/feed.xml
```
...
```text
https://video.juandecolonia.jc/video01.mp4
```
...
ffmpeg \
  ...
  -b:a 128k \
  -f flv \
  rtmp://video.juandecolonia.jc/live/ut8
```
...
```bash
curl -I http://video.juandecolonia.jc/hls/ut8.m3u8
```
...
Open Network Stream
  ↓
http://video.juandecolonia.jc/hls/ut8.m3u8
```
...
```bash
dig video.juandecolonia.jc
```

## TCP

```bash
nc -vz video.juandecolonia.jc 1935
nc -vz video.juandecolonia.jc 80
```

## HTTP

```bash
curl -I http://video.juandecolonia.jc/
```
...
```text
          ...
          ┌──────────┴──────────┐
          │                     │
      radio.juandecolonia.jc       video.juandecolonia.jc
          │                     │
          ▼                     ▼
       Icecast              Nginx/RTMP
          │                     │
